...
SELECT A.art_id, A.title, A.`series_num`, A.`edition_num`, A.`edition_num_max`, B.`location` FROM art AS A INNER JOIN art_locations AS B ON B.art_location_id = A.art_location_id WHERE A.title LIKE '%tinyviews%' ORDER BY A.edition_num ASC
