...
foreach ($fieldnotes_data as $key => $value) {
    ...
    $published_date =strtoupper ( date("F j\, Y", strtotime($value['created'])) );
    ...
    /* Check for image */
    if ( is_file($_SERVER['DOCUMENT_ROOT'] . "/view/__image/fieldnotes/" . $value['image'] ) ) {
        $img_html = '<img src="/view/__image/fieldnotes/' . $value['image'] . '" alt="' . $value['image'] . '" /></a>';
    ...
    if($img_div == '1') {
        $card_html .= '<div class="card--image-wrapper" style="background: rgba(0,0,0,0) url(/view/__image/fieldnotes/' . $value['image'] . ') no-repeat center; background-size: cover; word-break: break-word; box-shadow: 0px 20px 25px 0px rgba(0, 0, 0, 0.3);">';
