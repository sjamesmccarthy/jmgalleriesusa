...
$order_link = '<a target="_ao" href="/contact?photo=' . urlencode($res_currenOfferName) . '&size=SIZE-60CM&frame=STUDIO-SNOW+WHITE+or+ASH+GRAY&cost=' . $res_amazingOfferPrice . '&promo_code=ARTISTPROOF&email=' . $this->collector_data_obj->email . '&name=' . $this->collector_data_obj->first_name . ' ' . $this->collector_data_obj->last_name . '&catalog_no=' . $res_amazingOfferCatalogNumber . '">';
   ...
   <article id="rewards" class="mt-64">
        <div class="grid-4_sm-2 grid-4_md-3">
            <div class="most-popular--title col-12">
                ...
                <p class="mt-8">{$order_link}<b>{$res_amazingOfferPrice}</b> <strike>{$res_amazingOfferRegPrice}</strike> USD / Studio Framed (No Glass) / Artist Proof (2) / Paper / {$res_imageSize} / Signed with Certificate of Authenticity.  </a></p>
            </div>
