    ...
    foreach($catalog_names as $key=>$value) {
        ...
        if(strtolower($value['title']) != 'new releases') {
            ...
            foreach($catalog_photos as $k => $v) {
                ...
                $thumb_html .= '<a href="/' . $v['catalog_path'] . '/' . $img_file . '"><img style="width: 100%;" src="/view/__catalog/__thumbnail/' . $img_file . '.jpg" alt="' . $img_file . '" /></a></p><!--<p class="pt-8 blue text-center"><a href="' . $value['path'] . "/" . $img_file . '">' . $v['title'] . '</a></p>--><!--<p>' . $v['loc_place'] . '</p>--><!-- <p>Sizes: ' . $available_sizes . '</p>--><!--' . $desc_editions . '--></div>';
